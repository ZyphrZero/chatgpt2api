...
本文基于当前仓库当前实现整理，用于帮助用户快速了解哪些功能已经可用、哪些属于后续增强。

| 功能 | 状态 | 说明 |
|:----------------------------------------|:--:|:--------------------------------------------------------------|
| OpenAI 兼容 `POST /v1/images/generations` | ✅  | 已支持，用于图片生成，并可通过 `n` 返回多张图片。 |
| OpenAI 兼容 `POST /v1/images/edits` | ✅  | 已支持，可上传图片进行编辑。 |
| 面向图片工作流的 `POST /v1/chat/completions` | ✅  | 已支持图片生成请求；带图片的普通聊天会保留在多模态对话链路并路由到视觉账号。 |
| 面向图片工作流的 `POST /v1/responses` | ✅  | 已支持图片生成工具调用。 |
| 管理端异步创作任务 API | ✅  | 统一使用 `/api/creation-tasks` 资源根路径；提交子接口为 `image-generations`、`image-edits`、`chat-completions`，并支持任务轮询与取消。 |
| `GET /v1/models` 接口 | ✅  | 当前返回 `gpt-image-2`、`codex-gpt-image-2`、`auto`、`gpt-5-mini`、`gpt-5-3-mini`、`gpt-5`、`gpt-5-1`、`gpt-5-2`、`gpt-5-3`、`gpt-5.4`、`gpt-5.5`。 |
| 同时生成多张图片 | ✅  | 已支持，后端与前端都可进行多图生成。 |
| 前端创作台 | ✅  | 已支持手动切换对话 / 作画模式；作画模式支持图片生成、图片编辑、模型选择、历史记录与查看大图。 |
| 前端图片输入 / 参考图交互 | ✅  | 已支持参考图上传、预览、移除、编辑模式工作流，以及聊天模式下保留图片附件。 |
...
| 流式输出支持 | ✅  | 已实现。 |
| 图片尺寸参数 | ✅  | 已支持 `auto`、固定尺寸、比例和 `1080p` / `2k` / `4k` 等分辨率别名。 |
| 服务端图片 URL 缓存 | ✅  | 已实现。 |
| `rt_token` 刷新 | ⚠️ | 当前主链路使用 access token 与账号刷新；`rt_token` 自动换取 access token 属于后续增强。 |
| 代理配置功能 | ✅  | 已支持网页端配置全局 HTTP / HTTPS / SOCKS5 / SOCKS5H 代理，并应用到出站请求。 |
| Anthropic 协议支持 | ✅  | 已支持 `/v1/messages` 风格入口和流式事件转换。 |
